...
func expectedBytesString(v float64) string {
	i := 0
	for ; 1.0 <= v/1024.0; i++ {
		v /= 1024.0
	}
	...
	if i > 0 {
		return fmt.Sprintf("%.3f %ciB", v, unitStr[i])
	} else {
...
func TestBytes_Int(t *testing.T) {
	for _, expected := range tcValidNumberFormat {
		tested := Bytes(expected)
		assert.Equal(t, int64(expected), tested.Int())
	}
}
...
func TestBytes_String(t *testing.T) {
	for _, value := range tcValidNumberFormat {
		tested := Bytes(value)
		assert.Equal(t, expectedBytesString(float64(value)), tested.String())
	}
}
...
func TestBytes_UnmarshalJSON(t *testing.T) {
	...
	template := "{\"generated\":\"%s\"}"

	for in, expected := range tcValidNumberFormat {
		assert.NoError(t, json.Unmarshal([]byte(fmt.Sprintf(template, in)), &buffer))
		assert.Equal(t, Bytes(expected), buffer.Generated)
	}

	for _, in := range tcInvalidUnitFormat {
		assert.Error(t, json.Unmarshal([]byte(fmt.Sprintf(template, in)), &buffer))
	}
}

func TestBytes_MarshalJSON(t *testing.T) {
	buffer := &struct {
		Tested Bytes `json:"tested"`
	}{}
	template := "{\"tested\":\"%s\"}"

	for _, value := range tcValidNumberFormat {
		buffer.Tested = Bytes(value)
		generated, err := json.Marshal(buffer)

		assert.NoError(t, err)
		assert.Equal(t, fmt.Sprintf(template, expectedBytesString(float64(value))), string(generated))
	}
}
...
func TestBytes_UnmarshalYAML(t *testing.T) {
	...
	for in, expected := range tcValidNumberFormat {
		assert.NoError(t, yaml.Unmarshal([]byte(fmt.Sprintf(template, in)), &buffer))
		assert.Equal(t, Bytes(expected), buffer.Generated)
	}

	for _, in := range tcInvalidUnitFormat {
		assert.Error(t, yaml.Unmarshal([]byte(fmt.Sprintf(template, in)), &buffer))
	}
}

func TestBytes_MarshalYAML(t *testing.T) {
	buffer := &struct {
		Tested Bytes `yaml:"tested"`
	}{}
	template := "tested: %s\n"

	for _, value := range tcValidNumberFormat {
		buffer.Tested = Bytes(value)
		generated, err := yaml.Marshal(buffer)
